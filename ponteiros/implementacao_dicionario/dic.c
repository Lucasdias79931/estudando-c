...
void append(Dict *dict, const char *title) {
    if (!dict) return;

    bool keyfound = false;
    for (int item = 0; item < dict->lenKey; item++) {
        if (strcmp(title, dict->key[item]) == 0) {
            dict->value[item]++;
            keyfound = true;
            break;
        }
    ...
    if (!keyfound) {
        dict->lenKey++;
        dict->key = realloc(dict->key, dict->lenKey * sizeof(char *));
        dict->value = realloc(dict->value, dict->lenKey * sizeof(int));

        dict->key[dict->lenKey - 1] = malloc(strlen(title) + 1);
        strcpy(dict->key[dict->lenKey - 1], title);
        dict->value[dict->lenKey - 1] = 1;
    }
}
...
int main() {
    ...
    for (int i = 0; i < sizeof(titulos) / sizeof(titulos[0]); i++) {
        append(histograma, titulos[i]);
    }

  
    for (int i = 0; i < histograma->lenKey; i++) {
        printf("%s: %d\n", histograma->key[i], histograma->value[i]);
        free(histograma->key[i]);
    }

    free(histograma->key);
    free(histograma->value);
    free(histograma);
